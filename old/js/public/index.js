(() => {
  ...
  // Initialize services
  // CalendarService is now just a utility for dates? 
  // If it has DB dependency it needs refactor. But here we only use date logic.
  // We should check if we can just use it without DB or pass null.
  const calendarService = new CalendarService(null);
  const teamService = new TeamService(); // Supabase version
  ...
  async function initApp() {
    ...
    sb.auth.onAuthStateChange((event, session) => {
      ...
      if (teamFilter && filteredMatches.length > 0) {
        ...
        // Supabase matches have team_id. We need owner_id? 
        // We fetched matches with teams(owner_id).
        if (firstMatch.teams && firstMatch.teams.owner_id) {
          updateFollowButtonState(firstMatch.teams.owner_id, teamFilter);
        }
      }
    });
  ...
  async function loadMatches() {
    ...
    try {
      ...
      // We need team info (name, owner_id)
      ...
      if (matches) {
        ...
        allMatches = matches.map(m => ({
          ...
          fechaHora: m.date,
          equipoId: m.team_id,
          rivalId: null, // field might be missing or not used same way
          nombreEquipo: m.teams ? m.teams.name : 'Desconocido',
          nombreRival: m.rival_name,
          ownerUid: m.teams ? m.teams.owner_id : null,
          estado: m.state, // 'scheduled', 'live', 'finished'
          // Add other fields if MatchRenderer needs them
          pabellon: m.location, // For MatchRenderer
          location: m.location,
          localScore: m.team_score,
          puntosEquipo: m.team_score, // For MatchRenderer
          visitorScore: m.rival_score,
          puntosRival: m.rival_score, // For MatchRenderer
          isLocal: m.is_local, // if true, team is local
          teams: m.teams
        }));

        // Sort by date (already sorted by DB but just in case)
        allMatches.sort((a, b) => {
        ...
        if (teamFilter) {
          filteredMatches = allMatches.filter(m => m.equipoId === teamFilter);
          ...
          header.innerHTML = `Partidos Públicos <span class="badge bg-info fs-6">${escapeHtml(teamName)}</span> 
  ...
  async function updateFollowButtonState(ownerUid, teamId) {
    ...
    // teamService.isFollowing needs to be checked if it exists in Supabase version
    const isFollowing = await teamService.isFollowing(ownerUid, teamId, currentUser.id);
    if (isFollowing) {
  ...
  async function toggleFollow(ownerUid, teamId) {
    ...
    try {
      if (isFollowing) {
        await teamService.unfollowTeam(ownerUid, teamId, currentUser.id);
      } else {
        await teamService.followTeam(ownerUid, teamId, currentUser.id);
      }
  ...
  function renderMonthView() {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      dayMatches.forEach(match => {
        ...
        text.style.flex = '1';
        text.innerHTML = `<span class="fw-bold me-1">${time}</span> ${escapeHtml(match.nombreEquipo)} vs ${escapeHtml(match.nombreRival || 'Rival')}`;
  ...
  function renderDay(dayColumn, container, matches) {
    ...
    matches.forEach(match => {
      // Check if matchRenderer expects Supabase format or if we need to adapt
      // We already adapted in loadMatches, so it should be fine.
      const matchCard = matchRenderer.renderMatchCard(match, { isOwner: false });
